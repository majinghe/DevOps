...
安全永远是一个很重要的话题，以往的瀑布式开发中，安全是被滞后的（在测试甚至更靠后的阶段），留给安全的时间是相对比较充裕的；随着项目开发的敏捷化，早发布、快发布已经成为了主旋律，在这种情况下，安全需要被提前，也就是常说的安全左移（Security Shift Left），左移至编码，甚至计划阶段。
